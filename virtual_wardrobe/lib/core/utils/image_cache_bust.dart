...
/// `look-{lookId}`), for images a backend endpoint can overwrite in place at
/// their existing URL (see `OutfitService.regenerateLook`) — a plain
/// URL-keyed cache never sees that as a change, so any widget rendering
/// that image should fold [versionOf] into its cache key / Flutter `Key`.
///
...
/// (this page's own carousel, list thumbnails elsewhere, etc.) picks a
/// cache key that was never seen before — no manual cache eviction needed.
