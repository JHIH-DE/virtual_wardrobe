...
class LookService with BaseService {
  ...
  Future<List<Look>> getAllLooks() async {
    ...
    if (data is! List) {
      throw Exception('getAllLooks: response missing list data');
    }

    return data
        .whereType<Map<String, dynamic>>()
        .map((j) => Look.fromJson(j))
        .toList();
  }
  ...
  Future<List<Look>> getLooksByGarments(List<int> garmentIds) async {
    debugLog('--- getLooksByGarments: $garmentIds ---');
    ...
    final envelope = decodeMap(res, op: 'getLooksByGarments');
    final data = envelope['data'];
    if (data is! List) {
      throw Exception('getLooksByGarments: response missing list data');
    }
    return data
        .whereType<Map<String, dynamic>>()
        .map((j) => Look.fromJson(j))
