...
//
//  Vendored (with SharedMedia.swift and RSIComposeView.swift) from
//  receive_sharing_intent 1.9.0's
//  ios/receive_sharing_intent/Sources/receive_sharing_intent/RSIShareViewController.swift,
//  rather than linked via the package, because of a real conflict between
//  1.9.0's iOS distribution and this project:
//
//  - 1.9.0 dropped iOS CocoaPods support entirely (Swift-Package-Manager
//    only) — but this project can't go fully-SPM (flutter_facebook_auth /
//    fluttertoast / sign_in_with_apple don't support SPM yet), so linking
//    the package into *this* extension target either fails to compile
//    (`import Flutter` unresolved — the plugin package's own placeholder
//    Flutter module) or, once worked around via CocoaPods instead, drags in
//    ReceiveSharingIntentPlugin.swift (`import Flutter`, calls
//    AppDelegate-only APIs) as part of the same compiled unit, which is
//    fundamentally incompatible with an app-extension target
//    (APPLICATION_EXTENSION_API_ONLY).
//  - Downgrading the package to the last CocoaPods-era release (1.8.1)
//    sidesteps both of those, but 1.8.1's RSIShareViewController still
//    subclasses the now-deprecated `SLComposeServiceViewController` —
//    exactly what 1.9.0 rewrote away from — and testing on a real iPhone
//    (iOS 26) showed the resulting extension never gets registered by the
//    system at all (doesn't appear in *any* app's share sheet, not even
//    "More"; no crash log — it's a registration-time rejection, not a
//    runtime crash).
//
//  This file is the actual fix: 1.9.0's modern (plain UIViewController,
//  not SLComposeServiceViewController) implementation, compiled directly
//  into this target with no package/Flutter dependency at all — only
//  RSIShareViewController.swift, RSIComposeView.swift and SharedMedia.swift
//  are needed for that (verified via receive_sharing_intent's own source:
//  the only file in its iOS target that imports Flutter is
//  ReceiveSharingIntentPlugin.swift, the Flutter-plugin-registration half
//  Runner links via the real package — this class never calls it).
//  ShareViewController.swift (this target's actual NSExtensionMainStoryboard
//  class) subclasses this directly.
//
//  If upgrading receive_sharing_intent past 1.9.0 later, re-diff these
//  three files against the new version's source and update the vendored
//  copies to match — don't let them silently drift.
//
