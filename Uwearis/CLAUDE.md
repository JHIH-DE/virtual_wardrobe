...
- **Provider mutation methods**: verb + noun, never a bare verb — `addGarment`/`updateGarment`/`removeGarment`, not `add`/`update`/`remove`. This matters because these are called from other files, several layers away from the provider's own import; the noun disambiguates at the call site. (`trips_provider.dart`'s `add`/`remove` and `outfits_provider.dart`'s `add` are known migration debt against this rule — rename when next substantively touching those files, not as a standalone drive-by change.)
- **Debug logging**: see [Logging](#logging) below.
- **Categorical API values**: any fixed vocabulary of string values coming from the API (e.g. an outfit group's `type`) gets a Dart enum with `apiValue`/`fromApiValue(String)`, matching `GarmentCategory`/`StyleType`/`OccasionType`/`TripActivity`/`MatchALookRole`. Never compare a raw string literal against an API field. (`Outfit.groupType`, the very field this rule uses as its own example, doesn't yet have one — it's currently a raw `String` compared directly in a few pages. Known migration debt, not yet fixed.)
